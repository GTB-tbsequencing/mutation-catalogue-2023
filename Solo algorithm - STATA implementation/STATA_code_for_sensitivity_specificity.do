...
clear
import excel "List_of_graded_variants_25Apr2023.xlsx", sheet("Sheet1") firstrow allstring
...
save "Graded_mut_STATA_version2.dta", replace
...
* RIF genotypes without phenotypes are not yet included and will have to be merge in here. 
...
merge m:1 drug variant using "Graded_mut_STATA_version2.dta"
drop if _m==2
drop _m
...
*KAN: Ignore eis_c.-14C>T and eis_c.-10G>A if found in combination with LoF mutation in eis at a frequency >75%.
...
save "prepped_data_for_sens_spec_v2_`a'.dta", replace
}
...
cap erase "sens_spec_`c(current_date)'_v2.xlsx"
...
use "prepped_data_for_sens_spec_v2_`a'.dta", clear
...
export excel using "sens_spec_`c(current_date)'_v2.xlsx", firstrow(variables) sheet("group`q'_`a'_`y'") sheetmodify 
}
...
export excel using "sens_spec_`c(current_date)'_v2.xlsx", firstrow(variables) sheet("group12_`a'_`y'") sheetmodify 
...
export excel using "sens_spec_`c(current_date)'_v2.xlsx", firstrow(variables) sheet("group123_`a'_`y'") sheetmodify 
...
****************************************************************************************************
****************************************************************************************************
* For Catalogue Version 1
****************************************************************************************************
****************************************************************************************************

* Use v1 catalogue in new nomenclature. This is generated by 'convert_old_to_new_nomenclature_20042023':

set more off, permanently

* take all the data we used to derive the catalogue. This is an interim file from my main algorithm (the file of cleaned, merged data that we start with, before the samples with katG S315T and rpoB S450L pheno-S mutations are dropped). 

foreach a in 75 25 {
use "master_data_file_pheno_complete_with_orphans.dta", clear
keep sample_id drug  variant pheno maxaf effect				
gen het=1 if maxaf<.`a'
replace het=0 if het!=1

* merge in v2 catalogue:
merge m:1 drug variant using "v1_grades_for_sens_spec.dta"
drop if _m==2
drop _m
...
replace Initial_Confidence_Grading="Uncertain significance" if Initial_Confidence_Grading==""
...
replace rrdr=1 if strpos(variant,"rpoB_p.") & aa>=426 & aa<=452 & inlist(effect,"synonymous_variant","missing","initiator_codon_variant","stop_retained_variant")==0
replace rrdr=0 if rrdr!=1
replace Final_Confidence_Grading="2) Assoc w R - Interim" if Initial_Confidence_Grading=="Uncertain significance" & rrdr==1 & Final_Confidence_Grading!="1) Assoc w R"
drop aa

* if pooled LoF_all = AwR, then any LoF "Uncertain" mutation in the same drug_gene should be upgraded to AwR-interim for LoF_all for ETH-ethA; INH-katG; PZA-pncA; STM-gid, where LOF_all=LoF+inframe
replace Final_Confidence_Grading="2) Assoc w R - Interim" if Initial_Confidence_Grading=="Uncertain significance" & inlist(effect,"feature_ablation","frameshift","start_lost","stop_gained","inframe_deletion","inframe_insertion") & inlist(drug,"Ethionamide","Isoniazid","Pyrazinamide","Streptomycin") & inlist(gene,"ethA","katG","pncA","gid") &  het!=1 & Final_Confidence_Grading!="1) Assoc w R"
replace Final_Confidence_Grading="2) Assoc w R - Interim" if Initial_Confidence_Grading=="Uncertain significance" & inlist(effect,"frameshift","stop_gained") & drug=="Capreomycin" & gene=="tlyA" &  het!=1 & Final_Confidence_Grading!="1) Assoc w R"


*** EXPERT RULES END ***


** Generate prediction based on AwR ('group 1'):
gen g1=1 if Final_Confidence_Grading=="1) Assoc w R" & het==0
replace g1=0 if g1!=1
bysort sample_id drug: egen group1=max(g1)	
...
** Generate prediction based on AwRi only (ie strains don't already have an AwR mutation) ('group 2'):
gen g2=1 if Final_Confidence_Grading=="2) Assoc w R - Interim" & het==0
replace g2=0 if g2!=1
bysort sample_id drug: egen group2=max(g2)
replace group2=0 if group1==1
...
* reduce the data set to one row per sample_id / drug:
keep drug sample_id phenotype variant group1 group2 Grif
bysort sample_id drug: gen n=_n
keep if n==1
drop n sample_id variant
...
save "prepped_data_for_sens_spec_v1_`a'.dta", replace
}

** Two analyses follow:

** 1. Generate the outputs for maxaf .75 and .25; for rif-s rif-r rif-all; and for groups 1 and 2:

cap erase "sens_spec_`c(current_date)'_v1.xlsx"
...
use "prepped_data_for_sens_spec_v1_`a'.dta", clear
...
export excel using "sens_spec_`c(current_date)'_v1.xlsx", firstrow(variables) sheet("group`q'_`a'_`y'") sheetmodify 
}

}
}

** 2. Generate the outputs for maxaf .75 and .25; for rif-s rif-r rif-all; and for groups 1 and 2 combined:
...
export excel using "sens_spec_`c(current_date)'_v1.xlsx", firstrow(variables) sheet("group12_`a'_`y'") sheetmodify 
